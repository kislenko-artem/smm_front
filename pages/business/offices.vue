...
export default {
  ...
  methods: {
    ...
    getCategories(alias, model) {
      const self = this;
      fetch(process.env.baseUrl + "/v0/business/offices/" + alias)
        .then((response) => {
    ...
    addCategory(alias, model, name) {
      const self = this;
      fetch(process.env.baseUrl + "/v0/business/offices/offices", {
        method: 'POST',
    ...
    delCategory(alias, model, id) {
      const self = this;
      fetch(process.env.baseUrl + "/v0/business/offices/" + id, {
        method: 'DELETE',
      })
        .then((data) => {
          self.getCategories(alias, model);
